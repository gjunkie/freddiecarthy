...
async function handler(req, resp) {
  ...
  let articleLikes = await db
    .collection('likes')
    .findOne({ slug: requestBody.slug })
  ...
  if (
    ...
    (articleLikes.userLikes[hashedIp] &&
      articleLikes.userLikes[hashedIp] < LIKE_LIMIT_PER_USER)
  ) {
    // increment likes
    const articleData = await db.collection('likes').findOneAndUpdate(
      ...
      {
        $inc: {
          ['userLikes.' + hashedIp]: requestBody.likes,
          totalLikes: requestBody.likes,
        },
      },
