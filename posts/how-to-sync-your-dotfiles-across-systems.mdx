---
title: "How To Sync Your dotfiles Across Systems"
date: "2020-12-11"
...
In this series <strong>I'll show you how to supercharge your dotfiles to keep them synced across your machines effortlessly.</strong>

#### 1. Setting things up.
First things first you'll need to setup a repo on GitHub (or preferred location) for your dotfiles.
...
Make sure to set `GITHUB_USER` to your username and `GITHUB_REPO` to whatever you're calling your dotfiles repo. Let's also set up a couple of helper functions that will print out some fancy output as our script runs.
...
```
$ bash -c "$(curl -#fL raw.github.com/<your github username>/<your dotfiles repo name>/main/install)"
```

In the next _installments_ 😂 we'll expand on this by having it install our tooling as well as setting up an update script. That way, if we already have our dotfiles setup locally but they're outdated, we can easily update them.
